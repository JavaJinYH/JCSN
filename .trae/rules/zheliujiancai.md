...
## 一、核心开发原则
1. **唯一基准**：所有代码必须严格遵循项目文档 `docs/SPEC.md` 定义的架构、界面、数据模型，禁止私自变更。
2. **文档先行**：涉及数据库结构、核心业务、功能模块的大规模更新，**必须先修改 `docs/SPEC.md`**，标注版本号与更新日期，再编写代码。
...
4. 界面风格严格遵循 docs/SPEC.md 设计规范，不修改配色、布局
...
**同步内容**：
- 新架构、新数据模型 → 更新到 docs/SPEC.md 架构/数据模型章节
- 新业务规则 → 更新到 docs/SPEC.md 业务规则章节
- docs/SPEC.md 定位：项目**当前实际状态**快照，不是任务清单
