...
| 规则类别   | 规则内容         | 说明                         |
| ------ | ------------ | -------------------------- |
| **定价** | 每张订单保底10%利润  | 利润率 = (成交总额 - 成本总额) / 成交总额 |
| **收款** | 三类客户三种收款方式   | 业主现金/水电工年底对账/装修公司按项目对账     |
| **配送** | 根据利润决定是否免配送费 | 利润充足免费，极低不配送               |
| **退货** | 特殊商品按市场价退货   | 不锈钢、电线等贵金属按当日价格            |
...
**三类客户三种收款方式（2026-04-18更新）**：

| 客户类型     | 收款方式  | 说明                                 |
| -------- | ----- | ---------------------------------- |
| **业主**   | 现金现结  | 业主自行采购时当场付款，不欠账                    |
| **水电工**  | 年底对账  | 水电工负责多个小区项目，年底汇总各小区订单明细，微信发送账单对账催款 |
| **装修公司** | 按项目对账 | 对接公司老板，按具体项目/小区/房号整理订单，年底向装修公司老板收款 |
...
**根据利润决定是否免配送费（2026-04-18更新）**：

| 情况         | 配送费决策 | 说明               |
| ---------- | ----- | ---------------- |
| **利润充足**   | 免费配送  | 评估订单利润充足时免配送费    |
| **利润极低**   | 不配送   | 例如一捆线管仅赚5元，不值得配送 |
| **客户加付运费** | 可提供配送 | 客户愿意额外付运费，可以配送   |

**判断逻辑**：系统不做强制判断，由店员根据订单利润情况自行决定。
...
- 常规商品退货：按原进货单价结算
- **特殊商品退货（isPriceVolatile=true）**：按当日市场价格结算

**技术实现**：

- PurchaseReturn 模型：记录退货主信息
- PurchaseReturnItem 模型：记录退货明细（含 marketPrice 字段）
- 如果商品标记为价格波动，退货时弹窗让用户输入当日市场价

**退货流程**：

1. 选择原进货单
2. 选择要退货的商品
3. 如果商品 isPriceVolatile=true，弹窗输入当日市场价
4. 计算退货金额
5. 库存减少，应付款减少
...
3. 记住最近进价
4. 开单时如果成交价低于进价，给个警告
5. **每张订单保底10%利润**（新增核心规则）

**不需要**：毛利率系数、最低售价计算、权限控制等复杂逻辑。

### 8.1.1 保底利润规则（新增 2026-04-18）
...
1. 开单时实时计算 `(成交总额 - 成本总额) / 成交总额`
2. 如果利润率 < 10%，弹出警告："订单利润率低于10%，是否继续？"
3. 用户确认后可继续保存
...
**系统不做强制限制**，只提供警告提示，最终由店员判断是否接受订单。
...
| 商品类型     | 退货价格      | 说明                 |
| -------- | --------- | ------------------ |
| **常规商品** | 按原进货价结算   | 管道、水件等普通商品         |
| **特殊商品** | 按当日市场价格结算 | 不锈钢、电线等贵金属（价格每日波动） |
...
- 即使当日退货价高于原进货价，供应商也正常同意退货
- 系统记录原进货价和退货价，差异不计入损失

**系统实现**：

- 商品标记是否为"特殊商品"（价格波动标记）
- 退货时如果是特殊商品，弹出输入当日市场价格的对话框
- 退货金额按当日市场价格计算，而非原进货价
...
- Customers.tsx: ✅ 已删除（2026-04-18），迁移到 Contacts.tsx
- Customer 模型: ✅ 已删除（2026-04-18），数据已迁移到 Contact
...
**注意**：Customers.tsx 客户管理页面已废弃，所有客户数据已迁移到 Contacts.tsx
...
> **完整测试用例详见**：[TEST\_CASES.md](file:///d:/折柳建材/docs/TEST_CASES.md) — 包含约60个单功能测试、3个E2E全流程测试、10个异常处理测试
...
**全流程验证**（Phase 9 — 详见TEST\_CASES.md E2E章节）：

- [ ] E2E-001：完整业务周期测试（联系人→进货→销售→年终结算）
- [ ] E2E-002：退货全流程测试（销售退货+进货退货）
- [ ] E2E-003：新架构链路测试（联系人-主体-项目-销售）
...
> 注：P2-T1、P2-T2、P2-T5、P2-T6 已随 Customer→Contact 迁移完成
...
详细测试用例见：[TEST\_CASES.md](file:///d:/折柳建材/docs/TEST_CASES.md)

| 任务ID  | 任务名称   | 验证标准              | 对应测试用例                    |
| ----- | ------ | ----------------- | ------------------------- |
| P9-T1 | 全流程测试  | 无白屏、无错误           | E2E-001, E2E-002, E2E-003 |
| P9-T2 | 老数据兼容性 | 历史数据显示正常          | 详见TEST\_CASES.md第3.2章     |
| P9-T3 | 构建验证   | `npm run build`通过 | 独立验证                      |
...
3. **状态同步**：每次对话结束前更新 migration\_status.json
4. **验证优先**：Schema修改后立即 validate + generate + build
5. **文档记录**：遇到问题和解决方案都要记录到 migration\_status.json 的 notes 字段
